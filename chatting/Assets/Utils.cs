...
public class Utils
{
    ...
    {
        var controller = controllers.Find(pc => pc.gameObject != null);
    ...
    public static MethodInfo[] GetMethodsByPrefix(Type type, string methodPrefix)
    {
        ...
        {
            var methods = type.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance).Where(m => m.Name.StartsWith(methodPrefix)).ToArray();
            lookup[key] = methods;
        }
        return lookup[key];
    }
    ...
    {
        foreach (var method in GetMethodsByPrefix(type, methodPrefix))
